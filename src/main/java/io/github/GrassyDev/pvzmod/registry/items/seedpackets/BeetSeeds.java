...
public class BeetSeeds extends SeedItem implements FabricItem {
	public boolean used;
	public static int cooldown = (int) (PVZCONFIG.nestedSeeds.moreSeeds.beetS() * 20);

	public BeetSeeds(Settings settings) {
		super(settings);
	}
	...
	public void appendTooltip(ItemStack stack, @Nullable World world, List<Text> tooltip, TooltipContext context) {
		...
		tooltip.add(Text.translatable("item.pvzmod.beet_seed_packet.flavour")
	...
	public ActionResult useOnBlock(ItemUsageContext context) {
		...
		if (direction == Direction.DOWN) {
			...
			Box box = PvZEntity.BEET.getDimensions().getBoxAt(vec3d.getX(), vec3d.getY(), vec3d.getZ());
			if (world.isSpaceEmpty((Entity)null, box) && world instanceof ServerWorld serverWorld) {
				BeetEntity plantEntity = (BeetEntity) PvZEntity.BEET.create(serverWorld, itemStack.getNbt(), (Text) null, context.getPlayer(), blockPos, SpawnReason.SPAWN_EGG, true, true);
				List<PlantEntity> list = world.getNonSpectatingEntities(PlantEntity.class, PvZEntity.BEET.getDimensions().getBoxAt(plantEntity.getPos()));
				if (list.isEmpty()) {
	...
	public ActionResult useOnEntity(ItemStack stack, PlayerEntity user, LivingEntity entity, Hand hand) {
		...
		if (world instanceof ServerWorld serverWorld && entity instanceof TileEntity
			...
			BeetEntity plantEntity = PvZEntity.BEET.create(serverWorld, stack.getNbt(), (Text) null, user, blockPos, SpawnReason.SPAWN_EGG, true, true);
			List<PlantEntity> list = world.getNonSpectatingEntities(PlantEntity.class, PvZEntity.BEET.getDimensions().getBoxAt(plantEntity.getPos()));
			if (list.isEmpty()) {
			...
			BeetEntity plantEntity = (BeetEntity) PvZEntity.BEET.create(serverWorld, stack.getNbt(), (Text) null, user, entity.getBlockPos(), SpawnReason.SPAWN_EGG, true, true);
			if (plantEntity == null) {
				return ActionResult.FAIL;
			}
