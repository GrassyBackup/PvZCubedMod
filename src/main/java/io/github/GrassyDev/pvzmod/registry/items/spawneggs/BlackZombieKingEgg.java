...
public class BlackZombieKingEgg extends SeedItem {
    public BlackZombieKingEgg(Settings settings) {
        super(settings);
    }
    ...
    public ActionResult useOnBlock(ItemUsageContext context) {
        ...
        Box box = PvZEntity.BLACKZOMBIEKING.getDimensions().getBoxAt(vec3d.getX(), vec3d.getY(), vec3d.getZ());
             if (world.isSpaceEmpty((Entity)null, box) && world instanceof ServerWorld serverWorld) {
                    ZombieKingEntity zombieKingEntity = (ZombieKingEntity) PvZEntity.BLACKZOMBIEKING.create(serverWorld, itemStack.getNbt(), (Text) null, context.getPlayer(), blockPos, SpawnReason.SPAWN_EGG, true, true);
