...
public class BlackAnnouncerImpEgg extends SeedItem {
    public BlackAnnouncerImpEgg(Settings settings) {
        super(settings);
    }
    ...
    public ActionResult useOnBlock(ItemUsageContext context) {
        ...
        Box box = PvZEntity.BLACKANNOUNCERIMP.getDimensions().getBoxAt(vec3d.getX(), vec3d.getY(), vec3d.getZ());
             if (world.isSpaceEmpty((Entity)null, box) && world instanceof ServerWorld serverWorld) {
                    AnnouncerImpEntity impEntity = (AnnouncerImpEntity) PvZEntity.BLACKANNOUNCERIMP.create(serverWorld, itemStack.getNbt(), (Text) null, context.getPlayer(), blockPos, SpawnReason.SPAWN_EGG, true, true);
                    if (impEntity == null) {
