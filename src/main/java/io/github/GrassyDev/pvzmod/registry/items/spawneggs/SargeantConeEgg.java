...
public class SargeantConeEgg extends SeedItem {
    public SargeantConeEgg(Settings settings) {
        super(settings);
    }
	...
	public ActionResult useOnBlock(ItemUsageContext context) {
		...
		Box box = PvZEntity.SARGEANTBOWL.getDimensions().getBoxAt(vec3d.getX(), vec3d.getY(), vec3d.getZ());
		if (world.isSpaceEmpty((Entity) null, box) && world instanceof ServerWorld serverWorld) {
				SargeantEntity zombieEntity = (SargeantEntity) PvZEntity.SARGEANTBOWL.create(serverWorld, itemStack.getNbt(), (Text) null, context.getPlayer(), blockPos, SpawnReason.SPAWN_EGG, true, true);
