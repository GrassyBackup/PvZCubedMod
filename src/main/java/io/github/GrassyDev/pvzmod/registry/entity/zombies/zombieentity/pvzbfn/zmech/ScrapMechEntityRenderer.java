 ...
 */
public class ScrapMechEntityRenderer extends GeoEntityRenderer<ScrapMechEntity> {

    public ScrapMechEntityRenderer(EntityRendererFactory.Context ctx) {
		super(ctx, new ScrapMechEntityModel());
        this.shadowRadius = 1.5F; //change 0.7 to the desired shadow size.
    }

	@Override
	public void render(GeoModel model, ScrapMechEntity animatable, float partialTick, RenderLayer type, MatrixStack poseStack, @Nullable VertexConsumerProvider bufferSource, @Nullable VertexConsumer buffer, int packedLight, int packedOverlay, float red, float green, float blue, float alpha) {
