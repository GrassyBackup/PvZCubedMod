...
public class MeteorHammerSeeds extends SeedItem implements FabricItem {
	public boolean used;
	public static int cooldown = (int) (PVZCONFIG.nestedSeeds.moreSeeds.meteorhammerS() * 20);

	public MeteorHammerSeeds(Settings settings) {
		super(settings);
	}
	...
	public void appendTooltip(ItemStack stack, @Nullable World world, List<Text> tooltip, TooltipContext context) {
		...
		tooltip.add(Text.translatable("item.pvzmod.meteorhammer_seed_packet.flavour")
	...
	public ActionResult useOnBlock(ItemUsageContext context) {
		...
		if (direction == Direction.DOWN) {
			...
			Box box = PvZEntity.METEORHAMMER.getDimensions().getBoxAt(vec3d.getX(), vec3d.getY(), vec3d.getZ());
			if (world.isSpaceEmpty((Entity)null, box) && world instanceof ServerWorld serverWorld) {
				MeteorHammerEntity plantEntity = (MeteorHammerEntity) PvZEntity.METEORHAMMER.create(serverWorld, itemStack.getNbt(), (Text) null, context.getPlayer(), blockPos, SpawnReason.SPAWN_EGG, true, true);
				List<PlantEntity> list = world.getNonSpectatingEntities(PlantEntity.class, PvZEntity.METEORHAMMER.getDimensions().getBoxAt(plantEntity.getPos()));
				if (list.isEmpty()) {
	...
	public ActionResult useOnEntity(ItemStack stack, PlayerEntity user, LivingEntity entity, Hand hand) {
		...
		if (world instanceof ServerWorld serverWorld && entity instanceof TileEntity
			...
			MeteorHammerEntity plantEntity = PvZEntity.METEORHAMMER.create(serverWorld, stack.getNbt(), (Text) null, user, blockPos, SpawnReason.SPAWN_EGG, true, true);
			List<PlantEntity> list = world.getNonSpectatingEntities(PlantEntity.class, PvZEntity.METEORHAMMER.getDimensions().getBoxAt(plantEntity.getPos()));
			if (list.isEmpty()) {
			...
			MeteorHammerEntity plantEntity = (MeteorHammerEntity) PvZEntity.METEORHAMMER.create(serverWorld, stack.getNbt(), (Text) null, user, entity.getBlockPos(), SpawnReason.SPAWN_EGG, true, true);
			if (plantEntity == null) {
				return ActionResult.FAIL;
			}
