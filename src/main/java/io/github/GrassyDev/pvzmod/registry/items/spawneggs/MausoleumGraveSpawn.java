...
public class MausoleumGraveSpawn extends SeedItem {
    public MausoleumGraveSpawn(Settings settings) {
        super(settings);
    }
    ...
    public ActionResult useOnBlock(ItemUsageContext context) {
		...
		Box box = PvZEntity.MAUSOLEUMGRAVESTONE.getDimensions().getBoxAt(vec3d.getX(), vec3d.getY(), vec3d.getZ());
		if (world.isSpaceEmpty((Entity)null, box) && world instanceof ServerWorld serverWorld) {
                    MausoleumGraveEntity basicGraveEntity = (MausoleumGraveEntity) PvZEntity.MAUSOLEUMGRAVESTONE.create(serverWorld, itemStack.getNbt(), (Text) null, context.getPlayer(), blockPos, SpawnReason.SPAWN_EGG, true, true);
