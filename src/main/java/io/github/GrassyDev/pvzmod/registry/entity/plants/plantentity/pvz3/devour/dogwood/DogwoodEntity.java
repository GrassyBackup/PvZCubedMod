...
public class DogwoodEntity extends PlantEntity implements IAnimatable, RangedAttackMob {
	...
	public DogwoodEntity(EntityType<? extends DogwoodEntity> entityType, World world) {
	...
	private <P extends IAnimatable> PlayState predicate(AnimationEvent<P> event) {
		if (this.isFiring) {
			event.getController().setAnimation(new AnimationBuilder().playOnce("dogwood.bark"));
		} else {
			event.getController().setAnimation(new AnimationBuilder().loop("dogwood.idle"));
		}
		...
		if (this.age > 1) {
			...
			if ((!blockPos2.equals(blockPos) || !blockState.hasSolidTopSurface(world, this.getBlockPos(), this)) && !this.hasVehicle()) {
				if (!this.world.isClient && this.world.getGameRules().getBoolean(GameRules.DO_MOB_LOOT) && !this.naturalSpawn && this.age <= 10 && !this.dead){
					this.dropItem(ModItems.DOGWOOD_SEED_PACKET);
				}
				this.discard();
			}
	...
	public ActionResult interactMob(PlayerEntity player, Hand hand) {
		...
		if (itemStack.isOf(ModItems.GARDENINGGLOVE)) {
			dropItem(ModItems.DOGWOOD_SEED_PACKET);
	...
	public ItemStack getPickBlockStack() {
		return ModItems.DOGWOOD_SEED_PACKET.getDefaultStack();
	}
