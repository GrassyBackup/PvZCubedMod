...
public class ImpatyensSeeds extends SeedItem implements FabricItem {
	public static int cooldown = (int) (PVZCONFIG.nestedSeeds.moreSeeds.impatyensS() * 20);

	public ImpatyensSeeds(Settings settings) {
        super(settings);
    }
	...
	public void appendTooltip(ItemStack stack, @Nullable World world, List<Text> tooltip, TooltipContext context) {
		...
		tooltip.add(Text.translatable("item.pvzmod.impatyens_seed_packet.flavour")
				.formatted(Formatting.DARK_GRAY));

		tooltip.add(Text.translatable("item.pvzmod.impatyens_seed_packet.flavour2")
				.formatted(Formatting.DARK_GRAY));

		tooltip.add(Text.translatable("item.pvzmod.impatyens.alt")
        ...
        if (direction == Direction.DOWN) {
            ...
            Box box = PvZEntity.IMPATYENS.getDimensions().getBoxAt(vec3d.getX(), vec3d.getY(), vec3d.getZ());
             if (world.isSpaceEmpty((Entity)null, box) && world instanceof ServerWorld serverWorld) {
                    ImpatyensEntity plantEntity = (ImpatyensEntity) PvZEntity.IMPATYENS.create(serverWorld, itemStack.getNbt(), (Text) null, context.getPlayer(), blockPos, SpawnReason.SPAWN_EGG, true, true);
	...
	public ActionResult useOnEntity(ItemStack stack, PlayerEntity user, LivingEntity entity, Hand hand) {
		...
		if (world instanceof ServerWorld serverWorld) {
			plantEntity = PvZEntity.IMPATYENS.create(serverWorld, stack.getNbt(), (Text) null, user, blockPos, SpawnReason.SPAWN_EGG, true, true);
