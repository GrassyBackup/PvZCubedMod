...
public class WinterMelonEntity extends PlantEntity implements IAnimatable, RangedAttackMob {
    ...
    public WinterMelonEntity(EntityType<? extends WinterMelonEntity> entityType, World world) {
	...
	protected void initGoals() {
		this.goalSelector.add(1, new WinterMelonEntity.FireBeamGoal(this));
	}
		...
		if (tickDelay <= 1) {
			...
			if ((!blockPos2.equals(blockPos) || !blockState.hasSolidTopSurface(world, this.getBlockPos(), this)) && !this.hasVehicle()) {
				if (!this.getWorld().isClient && this.getWorld().getGameRules().getBoolean(GameRules.DO_MOB_LOOT) && !this.naturalSpawn && this.age <= 10 && !this.dead){
					this.dropItem(ModItems.WINTERMELON_SEED_PACKET);
				}
				this.discard();
			}
		}
	...
	public ActionResult interactMob(PlayerEntity player, Hand hand) {
		...
		if (itemStack.isOf(ModItems.GARDENINGGLOVE)) {
			dropItem(ModItems.WINTERMELON_SEED_PACKET);
	...
	public ItemStack getPickBlockStack() {
		return ModItems.WINTERMELON_SEED_PACKET.getDefaultStack();
	}


	/** /~*~//~*ATTRIBUTES*~//~*~/ **/

	public static DefaultAttributeContainer.Builder createWinterMelonAttributes() {
	...
	static class FireBeamGoal extends Goal {
		private final WinterMelonEntity plantEntity;
		private int beamTicks;
		private int animationTicks;

		public FireBeamGoal(WinterMelonEntity plantEntity) {
			...
			if ((!this.plantEntity.canSee(livingEntity)) &&
				...
				if (this.beamTicks >= 0) {
					...
					if (!this.plantEntity.isInsideWaterOrBubbleColumn()) {
						ShootingIceMelonEntity proj = new ShootingIceMelonEntity(PvZEntity.ICEMELON, this.plantEntity.world);
