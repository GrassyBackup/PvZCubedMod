...
public class UnicornGargantuarEgg extends SeedItem {
    public UnicornGargantuarEgg(Settings settings) {
        super(settings);
    }
    ...
    public ActionResult useOnBlock(ItemUsageContext context) {
        ...
        Box box = PvZEntity.UNICORNGARGANTUAR.getDimensions().getBoxAt(vec3d.getX(), vec3d.getY(), vec3d.getZ());
             if (world.isSpaceEmpty((Entity)null, box) && world instanceof ServerWorld serverWorld) {
                    GargantuarEntity gargantuarEntity = (GargantuarEntity) PvZEntity.UNICORNGARGANTUAR.create(serverWorld, itemStack.getNbt(), (Text) null, context.getPlayer(), blockPos, SpawnReason.SPAWN_EGG, true, true);
