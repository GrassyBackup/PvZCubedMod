...
public class GloomVineSeeds extends SeedItem implements FabricItem {
	public boolean used;
	public static int cooldown = (int) (PVZCONFIG.nestedSeeds.moreSeeds.gloomvineS() * 20);

	public GloomVineSeeds(Settings settings) {
		super(settings);
	}
	...
	public void appendTooltip(ItemStack stack, @Nullable World world, List<Text> tooltip, TooltipContext context) {
		...
		tooltip.add(Text.translatable("item.pvzmod.gloomvine_seed_packet.flavour")
				.formatted(Formatting.DARK_GRAY));

		tooltip.add(Text.translatable("item.pvzmod.gloomvine_seed_packet.flavour2")
				.formatted(Formatting.DARK_GRAY));
	}
	...
	public ActionResult useOnBlock(ItemUsageContext context) {
		...
		if (direction == Direction.DOWN) {
			...
			Box box = PvZEntity.GLOOMVINE.getDimensions().getBoxAt(vec3d.getX(), vec3d.getY(), vec3d.getZ());
			if (world.isSpaceEmpty((Entity)null, box) && world instanceof ServerWorld serverWorld) {
				GloomVineEntity plantEntity = (GloomVineEntity) PvZEntity.GLOOMVINE.create(serverWorld, itemStack.getNbt(), (Text) null, context.getPlayer(), blockPos, SpawnReason.SPAWN_EGG, true, true);
				List<PlantEntity> list = world.getNonSpectatingEntities(PlantEntity.class, PvZEntity.GLOOMVINE.getDimensions().getBoxAt(plantEntity.getPos()));
				if (list.isEmpty()) {
	...
	public ActionResult useOnEntity(ItemStack stack, PlayerEntity user, LivingEntity entity, Hand hand) {
		...
		if (world instanceof ServerWorld serverWorld && entity instanceof TileEntity
			...
			GloomVineEntity plantEntity = PvZEntity.GLOOMVINE.create(serverWorld, stack.getNbt(), (Text) null, user, blockPos, SpawnReason.SPAWN_EGG, true, true);
			List<PlantEntity> list = world.getNonSpectatingEntities(PlantEntity.class, PvZEntity.GLOOMVINE.getDimensions().getBoxAt(plantEntity.getPos()));
			if (list.isEmpty()) {
			...
			GloomVineEntity plantEntity = (GloomVineEntity) PvZEntity.GLOOMVINE.create(serverWorld, stack.getNbt(), (Text) null, user, entity.getBlockPos(), SpawnReason.SPAWN_EGG, true, true);
			if (plantEntity == null) {
				return ActionResult.FAIL;
			}
		...
		else if (world instanceof ServerWorld serverWorld && entity instanceof PlantEntity && !(entity instanceof PlantEntity.VineEntity) && !(entity instanceof CoconutCannonEntity) && !(entity.getVehicle() instanceof PlantEntity.VineEntity)){
			GloomVineEntity plantEntity = (GloomVineEntity) PvZEntity.GLOOMVINE.create(serverWorld, stack.getNbt(), (Text) null, user, entity.getBlockPos(), SpawnReason.SPAWN_EGG, true, true);
			if (plantEntity == null) {
				return ActionResult.FAIL;
			}
			...
			if (entity.hasVehicle()){
				plantEntity.startRiding(entity.getVehicle(), true);
			}
			entity.startRiding(plantEntity, true);
			world.playSound((PlayerEntity) null, plantEntity.getX(), plantEntity.getY(), plantEntity.getZ(), sound, SoundCategory.BLOCKS, 0.6f, 0.8F);
			if (!user.getAbilities().creativeMode) {
				if (!PVZCONFIG.nestedSeeds.infiniteSeeds() && !world.getGameRules().getBoolean(PvZCubed.INFINITE_SEEDS)) {
					stack.decrement(1);
