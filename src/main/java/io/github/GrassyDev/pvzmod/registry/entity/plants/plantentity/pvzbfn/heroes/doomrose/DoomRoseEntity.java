...
public class DoomRoseEntity extends PlantEntity implements IAnimatable, RangedAttackMob {
	...
	private <P extends IAnimatable> PlayState predicate(AnimationEvent<P> event) {
		...
		if (this.isFiring && !this.charge) {
			event.getController().setAnimation(new AnimationBuilder().playOnce("doomrose.start"));
		}  else if (this.isFiring) {
			event.getController().setAnimation(new AnimationBuilder().loop("doomrose.thump"));
		} else if (i <= 0) {
			event.getController().setAnimation(new AnimationBuilder().loop("doomrose.idle"));
		}
