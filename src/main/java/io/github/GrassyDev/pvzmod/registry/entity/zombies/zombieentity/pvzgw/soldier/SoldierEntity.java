...
public class SoldierEntity extends PvZombieEntity implements IAnimatable {
	...
	private <P extends IAnimatable> PlayState predicate(AnimationEvent<P> event) {
		if (this.isInsideWaterOrBubbleColumn()) {
			...
			if (inLaunchAnimation) {
				event.getController().setAnimation(new AnimationBuilder().loop("soldier.zpg"));
			}
	...
	public void tick() {
		...
		if (this.getAttacking() == null && !(this.getHypno()) && !this.world.isClient()){
			...
			if (this.CollidesWithPlant(1f, 0f) != null && !this.hasStatusEffect(PvZCubed.BOUNCED) && (this.onGround || this.isInsideWaterOrBubbleColumn())){
	...
	public boolean damage(DamageSource source, float amount) {
		if (!super.damage(source, amount)) {
			...
			if (this.getRecentDamageSource() == PvZCubed.HYPNO_DAMAGE && !(this.getHypno())) {
				...
				SoldierEntity hypnotizedZombie = (SoldierEntity) hypnoType.create(world);
