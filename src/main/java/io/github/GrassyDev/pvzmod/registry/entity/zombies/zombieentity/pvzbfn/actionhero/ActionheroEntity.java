...
public class ActionheroEntity extends BullyEntity implements IAnimatable {
	...
	private <P extends IAnimatable> PlayState predicate(AnimationEvent<P> event) {
		...
		if (this.riding){
			event.getController().setAnimation(new AnimationBuilder().loop("bully.rocketride"));
		}
		else if (this.isInsideWaterOrBubbleColumn()) {
			if (this.getPoleStage()) {
				event.getController().setAnimation(new AnimationBuilder().loop("bully.ducky.run"));
			}
			else {
			...
			if (!(event.getLimbSwingAmount() > -0.01F && event.getLimbSwingAmount() < 0.01F)) {
				if (this.getPoleStage()){
					event.getController().setAnimation(new AnimationBuilder().loop("bully.run"));
				}
				else {
