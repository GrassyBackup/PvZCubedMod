...
public class FairyTaleGraveSpawn extends SeedItem {
    public FairyTaleGraveSpawn(Settings settings) {
        super(settings);
    }
    ...
    public ActionResult useOnBlock(ItemUsageContext context) {
		...
		Box box = PvZEntity.FAIRYTALEGRAVESTONE.getDimensions().getBoxAt(vec3d.getX(), vec3d.getY(), vec3d.getZ());
		if (world.isSpaceEmpty((Entity)null, box) && world instanceof ServerWorld serverWorld) {
                    FairyTaleGraveEntity basicGraveEntity = (FairyTaleGraveEntity) PvZEntity.FAIRYTALEGRAVESTONE.create(serverWorld, itemStack.getNbt(), (Text) null, context.getPlayer(), blockPos, SpawnReason.SPAWN_EGG, true, true);
