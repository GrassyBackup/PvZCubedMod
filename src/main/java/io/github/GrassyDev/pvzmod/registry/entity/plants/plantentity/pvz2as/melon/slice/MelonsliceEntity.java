...
public class MelonsliceEntity extends PlantEntity implements IAnimatable, RangedAttackMob {
    ...
    public MelonsliceEntity(EntityType<? extends MelonsliceEntity> entityType, World world) {
	...
	protected void initGoals() {
		this.goalSelector.add(1, new MelonsliceEntity.FireBeamGoal(this));
	}
		...
		if (tickDelay <= 1) {
			...
			if ((!blockPos2.equals(blockPos) || !blockState.hasSolidTopSurface(world, this.getBlockPos(), this)) && !this.hasVehicle()) {
				if (!this.getWorld().isClient && this.getWorld().getGameRules().getBoolean(GameRules.DO_MOB_LOOT) && !this.naturalSpawn && this.age <= 10 && !this.dead){
					this.dropItem(ModItems.MELONSLICE_SEED_PACKET);
				}
				this.discard();
			}
		}
	...
	public ActionResult interactMob(PlayerEntity player, Hand hand) {
		...
		if (itemStack.isOf(ModItems.GARDENINGGLOVE)) {
			dropItem(ModItems.MELONSLICE_SEED_PACKET);
	...
	public ItemStack getPickBlockStack() {
		return ModItems.MELONSLICE_SEED_PACKET.getDefaultStack();
	}
	...
	static class FireBeamGoal extends Goal {
		private final MelonsliceEntity plantEntity;
		private int beamTicks;
		private int animationTicks;

		public FireBeamGoal(MelonsliceEntity plantEntity) {
			...
			if ((!this.plantEntity.canSee(livingEntity)) &&
				...
				if (this.beamTicks >= 0) {
					...
					if (!this.plantEntity.isInsideWaterOrBubbleColumn()) {
						ShootingSliceEntity proj = new ShootingSliceEntity(PvZEntity.SLICE, this.plantEntity.world);
