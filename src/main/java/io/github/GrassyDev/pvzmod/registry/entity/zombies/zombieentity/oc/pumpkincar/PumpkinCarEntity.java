...
public class PumpkinCarEntity extends ZombieVehicleEntity implements IAnimatable {
	...
	protected  <P extends IAnimatable> PlayState predicate(AnimationEvent<P> event) {
		if (!(event.getLimbSwingAmount() > -0.01F && event.getLimbSwingAmount() < 0.01F)) {
			event.getController().setAnimation(new AnimationBuilder().loop("pumpkincar.walking"));
		} else {
			event.getController().setAnimation(new AnimationBuilder().loop("pumpkincar.idle"));
		}
