...
public class SargeantBucketEgg extends SeedItem {
    public SargeantBucketEgg(Settings settings) {
        super(settings);
    }
	...
	public ActionResult useOnBlock(ItemUsageContext context) {
		...
		Box box = PvZEntity.SARGEANTHELMET.getDimensions().getBoxAt(vec3d.getX(), vec3d.getY(), vec3d.getZ());
		if (world.isSpaceEmpty((Entity) null, box) && world instanceof ServerWorld serverWorld) {
				SargeantEntity zombieEntity = (SargeantEntity) PvZEntity.SARGEANTHELMET.create(serverWorld, itemStack.getNbt(), (Text) null, context.getPlayer(), blockPos, SpawnReason.SPAWN_EGG, true, true);
