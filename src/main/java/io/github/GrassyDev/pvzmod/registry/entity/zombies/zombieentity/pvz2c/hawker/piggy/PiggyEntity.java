package io.github.GrassyDev.pvzmod.registry.entity.zombies.zombieentity.pvz2c.hawker.piggy;
...
public class PiggyEntity extends SmallAnimalEntity implements IAnimatable {

    private AnimationFactory factory = GeckoLibUtil.createFactory(this);
    private String controllerName = "walkingcontroller";

    public PiggyEntity(EntityType<? extends PiggyEntity> entityType, World world) {
	...
	private static final TrackedData<Integer> DATA_ID_TYPE_VARIANT =
			DataTracker.registerData(PiggyEntity.class, TrackedDataHandlerRegistry.INTEGER);
	...
	private <P extends IAnimatable> PlayState predicate(AnimationEvent<P> event) {
		if (this.isInsideWaterOrBubbleColumn()) {
			event.getController().setAnimation(new AnimationBuilder().loop("piggy.walk"));
			if (this.isIced) {
			...
			if (!(event.getLimbSwingAmount() > -0.01F && event.getLimbSwingAmount() < 0.01F)) {
				...
				event.getController().setAnimation(new AnimationBuilder().loop("piggy.idle"));
			}
		}
	...
	public ItemStack getPickBlockStack() {
		return ModItems.PIGGYEGG.getDefaultStack();
	}
	...
	protected void checkHypno(){
		hypnoType = PvZEntity.PIGGYHYPNO;
	}
	...
	public boolean damage(DamageSource source, float amount) {
		if (!super.damage(source, amount)) {
			...
			if (this.getRecentDamageSource() == PvZCubed.HYPNO_DAMAGE && !(this.getHypno())) {
				...
				PiggyEntity hypnotizedZombie = (PiggyEntity) hypnoType.create(world);
				hypnotizedZombie.refreshPositionAndAngles(this.getX(), this.getY(), this.getZ(), this.getYaw(), this.getPitch());
				hypnotizedZombie.initialize(serverWorld, world.getLocalDifficulty(hypnotizedZombie.getBlockPos()), SpawnReason.CONVERSION, (EntityData)null, (NbtCompound) null);
				hypnotizedZombie.setAiDisabled(this.isAiDisabled());
				hypnotizedZombie.setHealth(this.getHealth());
				PiggyEntity hypnoPassenger = null;
