...
public class ShootingFrisbeeEntity extends PvZProjectileEntity implements IAnimatable {
	...
	protected static final TrackedData<Boolean> RETURNING_TAG =
			DataTracker.registerData(ShootingFrisbeeEntity.class, TrackedDataHandlerRegistry.BOOLEAN);
	...
	public void setReturning(ShootingFrisbeeEntity.Returning returning) {
    ...
    public static final Identifier PacketID = new Identifier(PvZEntity.ModID, "frisbeeproj");

    public ShootingFrisbeeEntity(EntityType<? extends ThrownItemEntity> entityType, World world) {
        super(entityType, world);
		this.setNoGravity(true);
    }

    public ShootingFrisbeeEntity(World world, LivingEntity owner) {
		...
		while (true) {
			...
			if (!world.isClient && this.getReturning()) {
				if (entityStore.contains(entity) && this.retuningStart) {
					float damage = PVZCONFIG.nestedProjDMG.frisbeeDMG() * damageMultiplier;
