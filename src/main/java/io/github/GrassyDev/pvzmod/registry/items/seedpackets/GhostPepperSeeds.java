...
public class GhostPepperSeeds extends SeedItem implements FabricItem {
	public static int cooldown = (int) (PVZCONFIG.nestedSeeds.moreSeeds.ghostpepperS() * 20);
    public GhostPepperSeeds(Settings settings) {
        super(settings);
    }
	...
	public void appendTooltip(ItemStack stack, @Nullable World world, List<Text> tooltip, TooltipContext context) {
		...
		tooltip.add(Text.translatable("item.pvzmod.ghostpepper_seed_packet.flavour2")
				.formatted(Formatting.DARK_GRAY));
	}
		...
		if (hitResult.getType() == HitResult.Type.MISS) {
			...
			if (hitResult.getType() == HitResult.Type.BLOCK) {
				if (world instanceof ServerWorld serverWorld) {
					GhostpepperEntity aquaticEntity = this.createEntity(world, hitResult);
	...
	private GhostpepperEntity createEntity(World world, HitResult hitResult) {
		return (GhostpepperEntity)(new GhostpepperEntity(world, hitResult.getPos().x, hitResult.getPos().y, hitResult.getPos().z));
	}
	...
	public ActionResult useOnEntity(ItemStack stack, PlayerEntity user, LivingEntity entity, Hand hand) {
		...
		if (world instanceof ServerWorld serverWorld) {
			plantEntity = PvZEntity.GHOSTPEPPER.create(serverWorld, stack.getNbt(), (Text) null, user, blockPos, SpawnReason.SPAWN_EGG, true, true);
			list = world.getNonSpectatingEntities(PlantEntity.class, PvZEntity.GHOSTPEPPER.getDimensions().getBoxAt(plantEntity.getPos()));
		}
